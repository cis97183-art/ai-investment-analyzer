# --- AI Prompt 框架 (個股規則) ---
# V4 版: 已整合 FinMind 數據欄位與新聞情緒維度。
STOCK_PROMPT_FRAMEWORK = """
### 台股投資組合風險偏好定義規則 V4 (整合新聞情緒版)
...
| **6. 新聞情緒 (News Sentiment)** | **絕對優先**選擇近期新聞情緒為「正面」或「中性」的公司。 | **避免**選擇近期有明確「負面」新聞的公司。 | 可接受「中性」新聞，但需要有強勁的成長數據支撐。 |
"""

def get_data_driven_prompt_templates():
    """[更新版] 返回一個包含所有投資組合提示語模板的字典。"""
    
    templates = {
        "純個股": """
        你是一位專業的台灣股市投資組合經理。請根據「台股投資組合風險偏好定義規則 V4 (整合新聞情緒版)」以及使用者資訊，為他量身打造一個純台股的投資組合。
        **任務**: 從下方提供的「候選股票清單」中，挑選 5 到 8 支最符合 '{risk_profile}' 規則的台股，分配權重，並以指定的 JSON 格式回傳。
        **重點**: 你**必須**將每支股票的財務指標與「近期新聞情緒」進行綜合考量。
        **規則**: \n{stock_rules}
        
        **候選股票清單 (CSV格式)**:
        這是從本地資料庫經量化與情緒篩選後的優質候選名單，你**必須**從中挑選股票來建立投資組合。
        欄位包含: stock_id, stock_name, industry_category, pe_ratio, pb_ratio, yield, sentiment_category。
        \n{candidate_stocks_csv}

        **使用者資訊**: 風險偏好: {risk_profile}, 投入資金: {investment_amount}
        **你的輸出 JSON 中，'rationale' 欄位必須同時提及財務觀點與新聞情緒的影響。**
        **你的輸出必須是純粹的 JSON 格式，直接以 '{{' 開始，以 '}}' 結束。結構如下:**
        {{
          "summary": {{"title": "為{risk_profile}投資者設計的【純個股】投資組合", "overview": "這是一個根據您的風險偏好，並從量化與新聞情緒篩選後的優質名單中挑選出來的投資組合，旨在實現您的投資目標。", "generated_date": "{current_date}"}},
          "portfolio_metrics": {{
          ...
          "holdings": [
            {{"ticker": "<股票代碼>", "name": "<公司簡稱>", "industry": "<產業類別>", "weight": 0.25, "rationale": "<簡述選擇此股票的原因。範例: 選擇台積電，因其估值合理(P/E={pe_ratio})，且近期關於先進製程的新聞情緒正面，顯示技術領先地位穩固。>"}}
          ]
        }}
        """,
        "純 ETF": """
        ...
        **任務**:
        ...
        2. **衛星部位**: 從下方提供的「候選股票清單」中，為 '{risk_profile}' 風險偏好挑選 3-5 支個股。你**必須**將個股的財務指標與「近期新聞情緒」進行綜合考量。
        ...
        **候選股票清單 (CSV格式)**:
        這是從本地資料庫經量化與情緒篩選後的優質候選名單，你的衛星部位**必須**從中挑選股票來建立。
        欄位包含: stock_id, stock_name, industry_category, pe_ratio, pb_ratio, yield, sentiment_category。
        \n{candidate_stocks_csv}

        **使用者資訊**: 風險偏好: {risk_profile}, 投入資金: {investment_amount}
        **你的輸出 JSON 中，個股的 'rationale' 欄位必須同時提及財務觀點與新聞情緒的影響。**
        **你的輸出必須是純粹的 JSON 格式，直接以 '{{' 開始，以 '}}' 結束。結構如下:**
        **核心/衛星比例指引**: 保守型 (核心80%/衛星20%), 穩健型 (核心60%/衛星40%), 積極型 (核心40%/衛星60%)。
        {{
          ...
          "satellite_holdings": [
            {{"ticker": "2330", "name": "台積電", "weight": 0.15, "rationale": "全球半導體領導者，且近期新聞情緒正面，為衛星部位中追求成長的基石。"}}
          ]
        }}
        """
    }
